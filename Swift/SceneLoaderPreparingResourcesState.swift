    ...
    */
    private func loadResourcesAsynchronously() {
        ...
        let loadSceneOperation = NSBlockOperation {
            // Load a scene of the appropriate type into memory from file.
            switch sceneMetadata.sceneType {
                case is HomeEndScene.Type:
                    scene = HomeEndScene(fileNamed: sceneMetadata.fileName)
                case is LevelScene.Type:
                    scene = LevelScene(fileNamed: sceneMetadata.fileName)
                default:
                    fatalError("Unexpected scene type from scene metadata.")
            }
